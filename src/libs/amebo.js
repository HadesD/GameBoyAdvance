...
function haltSkip() {
  ...
  if (IORAM[0x07]&4) { //only check if clock is enabled.
    ...
    cycles = (256-IORAM[0x05])-1; //distance to timer overflow, minus 1 as the first tick's duration is NOT a full set of cycles, but instead a fraction.
  ...
  if (IORAM[0x40]&0x80) { //only if screen enabled
    ...
    cycles = ((linesToVBL-1)*456+(456-lineCycles))*CPUSpeed; //calculate cycles to next vbl, mul by CPUSpeed
    ...
    if (IORAM[0x41]&0x28) { //lcdstat modes 0 and 2 only fire when screen is not in vblank.
      ...
      if (IORAM[0x41]&0x20) { //mode 2: OAM interrupt
        if (IORAM[0x44]>143) {
          ...
          cycles = (456-lineCycles)*CPUSpeed;
          if (interruptCycles[1]>cycles) interruptCycles[1] = cycles;
        }
      }
    }
